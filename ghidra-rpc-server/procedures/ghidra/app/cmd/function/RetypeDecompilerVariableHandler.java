...
/**
 * Procedure RetypeDecompilerVariable: retype a decompiler-only local variable.
 *
 * <h2>What this does that {@code SetVariableDataTypeCmd} cannot</h2>
 *
 * <p>{@code SetVariableDataTypeCmd} (the CLI's {@code function variable
 * set-type}) requires a <b>stored</b> named variable: it looks the name up
 ...
 * {@code p_Var1} on a register temporary with no stored equivalent), no
 * stored variable exists, so {@code set-type} cannot reach them.
 *
 * <p>Ghidra's GUI <b>Retype Variable</b> (Ctrl-L) handles exactly this
 * case: it stores a dynamic-hash varnode override keyed by
 * {@code (function, pc, varnode hash)} via
 * {@link HighFunctionDBUtil#updateDBVariable} with a {@code null} name.
 * The same call works for both backing-Symbol HighSymbols and display-only
 * HighSymbols. This procedure exposes that override on the wire.
 *
 * <h2>Identification: two mutually-exclusive paths</h2>
 *
 * <ul>
 *   <li><b>{@code decompilerName}</b> — exact match in
 *       {@link LocalSymbolMap#getNameToSymbolMap()}. E.g. {@code puVar1}.
 ...
 *       {@code RetypeLocalAction}) to give {@code updateDBVariable} a clean
 *       target.</li>
 *   <li>Call {@link HighFunctionDBUtil#updateDBVariable} with {@code null}
 *       as the name (we are typing, not renaming) inside the
 ...
 *       updateDBVariable: propagated.</li>
 * </ul>
 */
public final class RetypeDecompilerVariableHandler implements RpcProcedure {
    ...
    public RpcResponse execute(JsonObject req, RpcContext ctx) throws Exception {
        ...
        DataType dt = ctx.requireDataType(RpcContext.reqStr(req, "dataType"));
        ...
        final DataType dtFinal = dt;
        final SourceType sourceFinal = source;
        final String targetName = target.getName();
        try {
            ctx.runWrite("Retype Decompiler Variable", () -> {
                HighFunctionDBUtil.updateDBVariable(writeFinal, null, dtFinal, sourceFinal);
            });
        } catch (Exception e) {
            return RpcResponse.error(
                "updateDBVariable failed for '" + targetName + "': " + e.getMessage());
        }
